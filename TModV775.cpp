////////////////////////////////////////////
// TModV775.cpp: Implementation of CAEN
// module V775AC
...
unsigned int TModV775::facqreg[2] = {0x100E, 0x0001};
unsigned int TModV775::fMaxDataVal = 4096;
...
static const int   DataValidMask=  0x4000;
...
ClassImp(TModV775)
  ...
  void TModV775::Initialization()
{
    his1d = new TH1*[fMaxChannel];
    for(unsigned int i=0; i<fMaxChannel; i++) his1d[i] = 0;
}

//DataReset of V775, do this after the writting of Geo to the module
int TModV775::DataReset()
{
...
int TModV775::Decode(unsigned int *&data_point)
{
    //CleanChData();
	...
	while( (dataMarker != DataEnderMask) && (nCh<=ChannelNums) ) //loop to the end of the data belongs to this module
	{
		...
		bool dunvalid = (tdata & UnderThrMask)||(tdata & OverflewMask)||(!(tdata & DataValidMask));
		if(!dunvalid)
		{
...
int TModV775::WriteCrateNumtoBoard(unsigned int cnum)
{
    if(cnum>MaxCrateNum)
    {
...
int TModV775::WriteGeoToBoard(unsigned int geo)
{
...
unsigned int TModV775::GetChannelData(int chnum)
{
