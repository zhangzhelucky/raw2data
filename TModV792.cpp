////////////////////////////////////////////
// TModV792.cpp: Implementation of CAEN
// module V792AC
...
unsigned int TModV792::facqreg[2] = {0x100E, 0x0001};
unsigned int TModV792::fMaxDataVal = 4096;
...
static const short ChNumbRsh=          16;
...
ClassImp(TModV792)
  ...
  void TModV792::Initialization()
{
    his1d = new TH1*[fMaxChannel];
    for(unsigned int i=0; i<fMaxChannel; i++) his1d[i] = 0;
    //chdata = new unsigned short[fMaxChannel];
}

//DataReset of V792, do this after the writting of Geo to the module
int TModV792::DataReset()
{
...
int TModV792::Decode(unsigned int *&data_point)
{
    //CleanChData();
    ...
    if(dataMarker == DataNotValidMask)
    {
	return -1;  //for not valid data
    }
	...
	{
	    //cout << "data val: " << ChannelNums << endl;
	    data_point++;
	    tdata  = (*data_point);
...
int TModV792::WriteCrateNumtoBoard(unsigned int cnum)
{
    if(cnum>MaxCrateNum)
    {
	cout << " Crate Number" << cnum << " > " << MaxCrateNum << endl;
	return 0;
    }
...
int TModV792::WriteGeoToBoard(unsigned int geo)
{
...
unsigned int TModV792::GetChannelData(int chnum)
{
